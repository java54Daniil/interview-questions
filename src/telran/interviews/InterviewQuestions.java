...
boolean res = true;
    ...
    if (word.length() != anagram.length() || word == anagram) {
        ...
        int[] charCount = new int[256]; // ASCII assumption
...
public static List<DateRole>assignRoleDates(List<DateRole> rolesHistory,
	...
	//create List<DateRole> with roles matching with the given
	//most effective structure
	List<DateRole> result = new ArrayList<>();
    for (LocalDate date : dates) {
        DateRole latestRole = null;
        for (DateRole role : rolesHistory) {
            if (role.date().isBefore(date) || role.date().equals(date)) {
                if (latestRole == null || role.date().isAfter(latestRole.date())) {
                    latestRole = role;
                }
            }
        }
        result.add(new DateRole(date, latestRole == null ? null : latestRole.role()));
    }
    return result;
	}
